...
<div class="invite_sent_wrapper">
    <div class="row">
        ...
        <div class="message-right-container p-4">
            ...
            <div class="row">
                <div class="col-12">
                    <x-view-profile-edit-card title="{{ __('message.invitations_received') }}" dataTarget="" noIconOrButton="true"
                                              :showBorderBottom="false" class="no-padding">
                        <div class="col-12">
                            <div class="row mb-4">
                                <div class="col-md-4">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <form action="{{route('supplier.delete.index.ticket')}}" method="post" id ="deleteform" class="d-none">
                                        @csrf
                                    ...
                                    <table class="table">
                                        ...
                                        @foreach($ticketinbox as $ticket)
                                            <tr class ="send-detasils" data-href="">
                                                ...
                                                <td><label  > {{ ++$i }} </label></td>
                                                <td> <a href="{{ route('supplier.ticket.index.detail',$ticket->id)}}">{{ $ticket->ticketreceiver->getSupplierFullName()}} </a> </td>
                                                <td>{{ $ticket->ticket_number}}</td>
                                                <td>{{$ticket->name}}</td>
                                                <td>@if($ticket->ticket_type==1)
                                                        Support
                                                ...
                                                <td>@if($ticket->attach_doc_id == null) none @else
                                                        <a href="{{asset('document/client/tickets'.'/'.$ticket->attachDoc->attach_doc_name)}}"> {{$ticket->attachDoc->attach_doc_name}}</a> @endif</td>
                                                <td>@if($ticket->status  == 1) active  @else Inactive @endif</td>
                                            </tr>
                                        @endforeach
                                        </tbody>
                                    </table>
